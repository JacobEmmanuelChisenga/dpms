                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @auth
                        ...
                        @can('viewAny', App\Models\Permit::class)
                            <x-nav-link :href="route('permits.index')" :active="request()->routeIs('permits.*') && ! request()->routeIs('permits.verify')">
                                {{ __('Permits') }}
                            </x-nav-link>
                        @endcan
                        @can('viewReports')
                        ...
                        @endcan
                    @endauth
                    <x-nav-link :href="route('permits.verify')" :active="request()->routeIs('permits.verify')">
                        {{ __('Verify permit') }}
                    </x-nav-link>
                </div>
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @auth
                ...
                @endcan
                @can('viewAny', App\Models\Permit::class)
                    <x-responsive-nav-link :href="route('permits.index')" :active="request()->routeIs('permits.*') && ! request()->routeIs('permits.verify')">
                        {{ __('Permits') }}
                    </x-responsive-nav-link>
                @endcan
                @can('viewReports')
                ...
                @endcan
            @endauth
            <x-responsive-nav-link :href="route('permits.verify')" :active="request()->routeIs('permits.verify')">
                {{ __('Verify permit') }}
            </x-responsive-nav-link>
        </div>
